  ...
  return (
    <section className="mb-16">
      ...
      <div className="bg-background/50 border border-border rounded-lg p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          {data.metrics?.length > 0 && (
            <div className="bg-primary/10 border border-border rounded-lg p-6">
              <h4 className="font-semibold text-primary mb-4">Data Points</h4>
                ...
                {data.metrics.map((metric, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-start gap-3">
                      <span className="font-mono text-primary flex-shrink-0">{label}</span>
                      <span className="portfolio-text-muted">{value}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {data.visuals?.length > 0 && (
            <div className="bg-primary/10 border border-border rounded-lg p-6">
              <h4 className="font-semibold text-primary mb-4">Visual Components</h4>
        ...
        {data.logic && (
          <div className="bg-primary/10 border border-border rounded-lg p-6">
            <h4 className="font-semibold text-primary mb-3">Implementation Logic</h4>
            <p className="text-foreground/80 text-sm leading-relaxed">{data.logic}</p>
          </div>
        )}
      </div>
    </section>
