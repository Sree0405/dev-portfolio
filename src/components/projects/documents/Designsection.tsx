    ...
    <section className="mb-16">
      ...
      <div className="bg-background/50 border border-border rounded-lg p-8">
        ...
        {data.themes?.length > 0 && (
          <div className="mb-6">
            <h4 className="font-semibold text-primary mb-3">Supported Themes</h4>
            <div className="flex flex-wrap gap-2">
              {data.themes.map((theme, i) => (
                <span
                  key={i}
                  className="px-4 py-2 bg-primary/10 border border-primary/30 rounded-full text-sm text-primary/80"
                >
                  {theme}
                </span>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.principles?.length > 0 && (
            <div className="bg-primary/10 border border-border rounded-lg p-6">
              <h4 className="font-semibold text-primary mb-3">Design Principles</h4>
          ...
          {data.patterns?.length > 0 && (
            <div className="bg-primary/10 border border-border rounded-lg p-6">
              <h4 className="font-semibold text-primary mb-3">UI Component Patterns</h4>
                ...
                {data.patterns.map((pattern, i) => {
                  ...
                  return (
                    <div key={i}>
                      ...
                      {desc && <span className="portfolio-text-muted"> — {desc}</span>}
                    </div>
                  );
                })}
              </div>
