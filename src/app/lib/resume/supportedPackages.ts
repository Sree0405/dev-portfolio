...
export interface ResumeSupportedPackage {
  /** LaTeX package name passed to \\usepackage{...} */
  name: string;
  /** Typical options used in the default template preamble */
  options?: string;
  /** How the in-browser latex.js preview handles this package */
  preview: ResumePackagePreviewMode;
  note: string;
}

/**
 * Curated package set for resume templates.
 * PDF compile (server) uses packages natively; browser preview emulates or replaces them.
 */
...
/** Standard preamble — only use packages from RESUME_SUPPORTED_PACKAGES. */
